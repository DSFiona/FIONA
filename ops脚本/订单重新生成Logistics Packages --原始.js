    ...
    function (record, search, log, task) {
        ...
        function reduce(context) {
            ...
            try {
                ...
                if (!logistics_account) return;
                var tailLogistics = newRec.getValue("custbody94")
                //计算当前订单的总产品数量
...
//                    log.debug("tailLogistics", tailLogistics);
                    ...
                    if (itemType == 'Kit') {
                        ...
                        tailLogistics = kitItem.getValue("custitem87");
            ...
            for (var k = 0; k < item_qty; k++) {
                ...
                for (var i = 1; i <= 5; i++) {
                    ...
                    if (length && width && height && weight) {
                        ...
                        packageRec.setText("custrecord1515", tailLogistics); // 设置包裹记录的尾部物流信息
                        if (location) {
                            packageRec.setValue("custrecord_logistics_packages_location", location);
                        } // 如果仓库存在，设置包裹记录的位置
                        packageRec.setValue("custrecord_itme_line_key", lineId); // 设置包裹记录的行ID键
                        packageRec.setValue("custrecord_logistics_urgent_express", urgentExpress);//2024.9.25新增-获取加急运输
                        packageRec.setValue("custrecord_fulfillment_order_lineitem_id",custcol_aio_order_item_id) // 设置包裹记录的履行订单行项目ID
                        packageRec.setValue("custrecord_logistics_packages_item_type", itemType); // 设置包裹记录的物品类型

                        var logLogisticsPackagesId = packageRec.save({ignoreMandatoryFields: true}); // 保存包裹记录并忽略必填字段
                        log.debug(logLogisticsPackagesId, 'line:' + line); // 输出调试信息，包括保存的包裹记录ID和行号
                        line++;
                    }
                }
            }
