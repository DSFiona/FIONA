...
define([
			...
			datalist.push(
				{
					...
					kitRate: rec.getValue(rec.columns[3]),// 单价
					kitQuantity: rec.getValue(rec.columns[4]), //数量
	...
	function reduce(context) {
		try {
            ...
            if (!found) {
					log.warning('未找到子件批发价，使用套件单价', {
						item: memberItem,
						kitRate: kitRate
					});
					wholesalePrices[memberItem] = kitRate / memberQuantities[index];
					totalWholesaleValue += kitRate;
				};
